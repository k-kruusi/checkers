import { Coord, Piece, TileData, GamePhase, down, up } from "../../schema";
import {
  equalCoords, getDirectionFor, isBlack, isInBounds, isInRouteAlready,
  isKing, isOccupiedByOpponent, isRed, isTileActive, kingMe
} from "../tileUtilities";


describe('kingMe function', () => {
  it('should log an error for Piece.Empty', () => {
    expect(kingMe(Piece.Empty)).toBe(Piece.Empty);
  });
...
describe('isOccupiedByOpponent function', () => {
  ...
  it('should return false if the other piece is not an opponent piece', () => {
    ...
    expect(isOccupiedByOpponent(Piece.Red, Piece.RedKing)).toBe(false);
...
describe('getDirectionFor function', () => {
  ...
  it('should filter out the last back direction for kings', () => {
    ...
    const expectedDirections: Coord[] = [{ x: 1, y: -1 }, { x: -1, y: 1 }, { x: 1, y: 1 }];
...
describe('isTileActive function', () => {
  it('should return true for empty tile when not in transition', () => {
    expect(isTileActive(GamePhase.Black, Piece.Empty)).toBe(true);
    expect(isTileActive(GamePhase.Red, Piece.Empty)).toBe(true);
  });

  it('should return true for non-empty tile when in transition', () => {
    expect(isTileActive(GamePhase.TransitionToBlack, Piece.Black)).toBe(true);
    expect(isTileActive(GamePhase.TransitionToRed, Piece.Red)).toBe(true);
  });

  it('should return true for Black piece on Black turn', () => {
    expect(isTileActive(GamePhase.Black, Piece.Black)).toBe(true);
  });

  it('should return false for Red piece on Black turn', () => {
    expect(isTileActive(GamePhase.Black, Piece.Red)).toBe(false);
  });

  it('should return true for Red piece on Red turn', () => {
    expect(isTileActive(GamePhase.Red, Piece.Red)).toBe(true);
  });

  it('should return false for Black piece on Red turn', () => {
    expect(isTileActive(GamePhase.Red, Piece.Black)).toBe(false);
  });

  it('should return true for BlackKing piece on Black turn', () => {
    expect(isTileActive(GamePhase.Black, Piece.BlackKing)).toBe(true);
  });

  it('should return true for RedKing piece on Red turn', () => {
    expect(isTileActive(GamePhase.Red, Piece.RedKing)).toBe(true);
  });

  it('should return false for BlackKing piece on Red turn', () => {
    expect(isTileActive(GamePhase.Red, Piece.BlackKing)).toBe(false);
  });

  it('should return false for RedKing piece on Black turn', () => {
    expect(isTileActive(GamePhase.Black, Piece.RedKing)).toBe(false);
  });
});